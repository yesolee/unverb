...
Semantic Scholar API와 OpenAlex API를 이용하여 논문을 수집한다.
"""
...
def search_semantic_scholar(keyword: str, retry_count: int = 0) -> List[Dict]:
    ...
    try:
        ...
        search_stats[keyword] += len(papers)
...
def search_openalex(keyword: str, retry_count: int = 0) -> List[Dict]:
    ...
    try:
        log_message(f"OpenAlex 검색 중... [{keyword}]")
        total_requests += 1
        ...
        log_message(f"[ERROR] OpenAlex - {keyword}: {str(e)}")
        if retry_count < 3:
            log_message(f"  → 재시도 ({retry_count + 1}/3)")
            rate_limit_delay(2)
            return search_openalex(keyword, retry_count + 1)
...
def filter_openalex_paper(paper: Dict, keyword: str) -> Optional[Dict]:
    ...
    return {
        ...
        "doi": doi,
        "citation_count": citation_count,
        "abstract": abstract,  # 나중에 Semantic Scholar에서 채움
...
def collect_papers():
    """
    Primary 키워드로 Semantic Scholar 검색
    필요시 OpenAlex 보충 검색
    """
    log_message("=" * 60)
    log_message("논문 수집 시작")
    log_message("=" * 60)
    ...
    # Step 2: 부족시 Secondary 키워드로 추가 검색
    current_count = len(papers_by_doi)
    log_message("")
    log_message("[Step 2] 보충 검색 (필요시)")
    log_message("-" * 60)
    ...
    # Step 3: Abstract 보충 (OpenAlex에서 가져온 논문)
    log_message("")
    ...
    final_missing_abstract = sum(1 for p in final_papers if not p.get("abstract"))
    if final_missing_abstract > 0:
        log_message(f"  - Abstract 누락: {final_missing_abstract}편")
